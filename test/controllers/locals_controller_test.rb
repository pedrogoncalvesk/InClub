...
class LocalsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create local" do
    assert_difference('Local.count') do
      post locals_url, params: { local: { description: @local.description, lat: @local.lat, long: @local.long, name: @local.name, status: @local.status } }
    end
  ...
  test "should update local" do
    patch local_url(@local), params: { local: { description: @local.description, lat: @local.lat, long: @local.long, name: @local.name, status: @local.status } }
